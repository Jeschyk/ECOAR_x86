...
imgInfo* LineToAsm(imgInfo* pImg, int x, int y);
imgInfo* MoveToAsm(imgInfo* pImg, int x, int y);
imgInfo* SetColorAsm(imgInfo* pImg, int col);
...
void DrawPolyCircle(imgInfo* pInfo, int vCnt, int nStep, int cX, int cY, int radius)
{
	...
	MoveToAsm(pInfo, cX, cY - radius); //MoveTo(pInfo, cX, cY - radius);
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	for (i=0; i<256; ++i)
	{
		MoveToAsm(pInfo, 256, i); //MoveTo(pInfo, 256, i);
		LineToAsm(pInfo, 511, i); //LineTo(pInfo, 511, i);
		MoveToAsm(pInfo, i, 256); //MoveTo(pInfo, i, 256);
		LineToAsm(pInfo, i, 511); //LineTo(pInfo, i, 511);
	}
